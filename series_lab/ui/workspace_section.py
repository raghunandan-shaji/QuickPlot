...
def render_workspace() -> None:
    ...
    for index, (key, item) in enumerate(list(workspace.items())):
        c1, c2, c3, c4, c5 = st.columns([0.11, 0.45, 0.18, 0.17, 0.09], vertical_alignment="center")
        with c1:
            if st.button("hide" if item.visible else "show", key=f"visibility_{index}", help="Hide from or show on charts"):
                ...
                st.rerun()
        with c2:
            st.markdown(identity_html(key, item.fetched.title, muted=not item.visible), unsafe_allow_html=True)
        with c3:
